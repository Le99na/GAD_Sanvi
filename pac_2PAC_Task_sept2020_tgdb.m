function pac_2PAC_Task_sept2020_tgdb(subIdx,varargin)
...
ROI_DEF = {...
    ...
    'rPPC',   [77 78 84 85 86 90 91 92]};
numROIs = length(ROI_DEF) / 2;
ROI_NAMES = ROI_DEF(1:2:(numROIs*2));
...
canonical_SEED_ANALYSIS = {'aPFC_theta_gamma','cPFC_delta_beta'};
canonical_analysisWindow = {[0.6 1.2],  [0.2 1.4]};
numSeedROIs_canonical = length(canonical_SEED_ANALYSIS);
...
numCycles_low = 3;
numCycles_high = 5;
exhaustive_SEED_TARGET = {'aPFC_rPPC','cPFC_rM1'};%'cPFC_cPFC','aPFC_aPFC'};
exhaustive_analysisWindow = {[0.6 1.2],  [0.2 1.4]}; % old: aPFC version [0.4 1.6]
...
for seedIdx = 1:numSeedROIs_canonical
    canonical_analysis = canonical_SEED_ANALYSIS{seedIdx};
    canonicalAnalysis_parts = regexp(canonical_analysis,'_','split');
    seedRoi = canonicalAnalysis_parts{1};
    pacPair = [canonicalAnalysis_parts{2} '_' canonicalAnalysis_parts{3}];
    analysisWindow = canonical_analysisWindow{seedIdx};

    % Seed roi channels
    roiIdx = find(strcmp(seedRoi,ROI_DEF),1);
    roi_chanIdxs = ROI_DEF{roiIdx+1};

    canonicalPac_subjectFile = sprintf('%salt_seedPac_%s_%s_%s_%iiter_%s.mat',...
        PAC_OUTPUT,subject,seedRoi,pacPair,numIterations,analysisVersion);
...
end % loop canonical seed analyses
...
for pairRoiIdx = 1:numPairsROI_exhaustive
    ...
    exhaustivePac_subjectFile = sprintf('%salt_exhPac_%s_seed%s_target%s_%iiter_%inumFreq_%s.mat',...
        PAC_OUTPUT,subject,seedRoi,targetRoi,numIterations,numFreqs,analysisVersion);
    ...
    if exist(exhaustivePac_subjectFile,'file')~=2
        ...
        for condIdx = 1:numCond
            ...
            for lowFreqIdx = 1:numLowFreq
                ...
                analysisWindow = exhaustive_analysisWindow{pairRoiIdx};
