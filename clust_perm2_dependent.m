...
function [pval, t_orig, clust_info, seed_state, est_alpha]=clust_perm2_dependent(dataA,dataB,chan_hood,n_perm,fwer,tail,thresh_p,verblevel,seed_state,freq_domain)
    ...
    % Check that dataA and dataB have the same number of participants
    [n_chan, n_tpt, n_subsA] = size(dataA);
    [n_chan2, n_tpt2, n_subsB] = size(dataB);
    if n_subsA ~= n_subsB
        error('The number of participants must be the same for both conditions.');
    end
    ...
    df = n_subsA - 1;
    mult_fact = 1 / n_subsA; % change the factor for paired samples
    if tail
        %one tailed test
    ...
    for perm=1:n_perm
        % Randomly choose participants to flip the sign of the difference
        flip_signs = rand(1, total_subs) > 0.5; % binary array indicating which participants to flip
        
        % Compute the difference between conditions for each participant
        diff_data = dataA - dataB;
        
        % Apply the sign flipping to the difference data
        diff_data(:,:,flip_signs) = -diff_data(:,:,flip_signs);
        
        % Recreate the permuted data for condition A by adding/subtracting half of the difference
        data_perm = (dataA + dataB) / 2 + diff_data / 2;
            
        %compute t-scores
        all_t = paired_tmax(data_perm, dataB, n_subsA, df, mult_fact);
    ...
    % Compute original t-scores for the real data
    t_orig = paired_tmax(dataA, dataB, n_subsA, df, mult_fact);
